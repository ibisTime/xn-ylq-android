...
//        });
        setSubRightTitle("借款记录", getResources().getColor(R.color.title_bg), v -> {
            if (!SPUtilHelpr.isLogin(mActivity, false)) {
                return;
            }
...
//        data.setUserProductStatus("0");//删除删除
        if (data == null) {
            return;
        }
        //现在只需要判断是不是  锁定状态,不是状态就直接进入  其余的状态不需要判断
        if (TextUtils.equals("1", data.getIsLocked())) {//锁中状
