 ...
 */
public class MyScrollPicker extends View{
    ...
    private List<TimeModel> mInitData;//初始化数据
    ...
    public MyScrollPicker(Context context, AttributeSet attrs) {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mVisibleItemCount = typedArray.getInt(
                    ...
                    mVisibleItemCount);
            mIsCirculation = typedArray.getBoolean(R.styleable.ScrollPicker_is_circulation, false);
            typedArray.recycle();
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        drawItem(canvas, mSelected);         //绘制当前选中显示的数据
        int length = mVisibleItemCount / 2 + 1;
        for (int i = 1; i <= length && i <= mData.size() / 2; i++) {
            drawItem(canvas, mSelected - i);       //绘制选中显示之前的数据
            drawItem(canvas, mSelected + i);       //绘制选中显示后前的数据
        }
    }
    ...
    private void drawItem(Canvas canvas, int position) {
        ...
        TimeModel model = mData.get(position);
        if( model == null )
            return;
        String text = model.toString()+model.getWeek();
        float x = 0;
        if (relative == -1) { // 上一个
...
//        computeColor(relative);
        canvas.drawText(text, x, y + mMoveLength, mPaint);  //mMoveLength 滑动时有移动效果
    }
...
//        }
    ...
    public interface OnSelectedListener {
        void onSelected(List<TimeModel> data, int position);
    }
}
