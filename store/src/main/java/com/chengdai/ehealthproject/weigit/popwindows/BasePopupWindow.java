...
public abstract class BasePopupWindow implements BasePopup {
    ...
    private void initView(Activity context, int w, int h) {
        ...
        //默认占满全屏
        mPopupWindow = new CustomPopupWindow(mPopupView, w, h);
        //    mPopupWindow = new PopupWindow(mPopupView, w,WindowManager.LayoutParams.WRAP_CONTENT);
        //    mPopupWindow = new PopupWindow(mPopupView, w,context.getWindow().getDecorView().getWidth()/2+50);
        //指定透明背景，back键相关
    ...
    public void setAnimotionStyle(int animotion) {
        mPopupWindow.setAnimationStyle(animotion);
    }
...
//                LogUtil.I(location[1]+"  : height = "+mPopupView.getMeasuredHeight());
    ...
    public void showTopPopupWindow(int res, int x, int y) {
        try {
            if (res != 0) {
    ...
    private void tryToShowPopup(int res, View v) throws Exception {
        ...
        //传递了res
        if (res != 0 && v == null) {
            mPopupWindow.showAtLocation(mContext.findViewById(res), Gravity.BOTTOM, 0, 0);
        }
        //什么都没传递，取顶级view的id
        if (res == 0 && v == null) {
            mPopupWindow.showAtLocation(mContext.findViewById(android.R.id.content), Gravity.BOTTOM, 0, 0);
        }
